...
import { Users, Building2, Calendar, CreditCard, ArrowRightLeft, DollarSign, MapPin, UserPlus, TrendingUp, ChevronDown, ChevronRight, LayoutDashboard } from "lucide-react";
...
const AdminSidebar: React.FC<AdminSidebarProps> = ({
  ...
  return <div className="w-72 bg-white border-r border-gray-200 min-h-screen p-6">
      <div className="mb-8">
      ...
      <nav className="space-y-2">
        {sidebarItems.map(item => <div key={item.id}>
            {item.hasSubmenu ? <Collapsible>
                <CollapsibleTrigger asChild>
                  <Button variant="ghost" className={cn("w-full justify-between text-left", expandedItems.includes(item.id) && "bg-gray-100")} onClick={() => onToggleExpand(item.id)}>
